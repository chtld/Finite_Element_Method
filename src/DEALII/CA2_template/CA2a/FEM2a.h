...
class FEM
{
 public:
  ...
  //righthandside and boundary function
  double righthandside(std::vector<double> x);
  double dirichlet_boundary(std::vector<double> x);
  double neumann_boundary(std::vector<double> x);
...
double FEM<dim>::basis_function(unsigned int node, double xi_1, double xi_2){
  ...
  if(node == 0){
  	...
  	std::cout << "no such basis!" << std::endl;
  }
  //EDIT
...
std::vector<double> FEM<dim>::basis_gradient(unsigned int node, double xi_1, double xi_2){
  ...
  if(node == 0){
  	values[0] = -1.0/4.0*(1-xi_2);
  	values[1] = -1.0/4.0*(1-xi_1);
  }else if(node == 1)
  {
  	values[0] = 1.0/4.0*(1-xi_2);
  	values[1] = -1.0/4.0*(1+xi_1);
  	...
  	std::cout << "no such basis gradient!" << std::endl;
  }
  //EDIT
...
void FEM<dim>::define_boundary_conds(){
  ...
  for(unsigned int i = 0; i < totalNodes; i++){
  	if(nodeLocation[i][1] == 0. || nodeLocation[i][1]==0.08){
  		x[0] = nodeLocation[i][0];
      x[1] = nodeLocation[i][1];
  		boundary_values[i] = dirichlet_boundary(x);
  	}
  }
}
  ...
  for (;elem!=endc; ++elem){
    ...
    for(unsigned int q1=0; q1<quadRule; q1++){
      for(unsigned int q2=0; q2<quadRule; q2++){
        ...
        for(unsigned int A=0; A<dofs_per_elem; A++){
          for(unsigned int i=0; i<dim; i++){
            x[i] += nodeLocation[local_dof_indices[A]][i]*basis_function(A, quad_points[q1], quad_points[q2]);
          } 
        }
          ...
          f = righthandside(x);
	        Flocal(A) += f*detJ*basis_function(A, quad_points[q1], quad_points[q2]);
	      }
      }
    }
	        ...
	        for(unsigned int B=0; B<dofs_per_elem; B++){
	          for(unsigned int i=0;i<dim;i++){
	            for(unsigned int j=0;j<dim;j++){
                    ...
                    Klocal(A, B) += -basis_gradient(A, quad_points[q1],quad_points[q2])[I]
...
template <int dim>
double FEM<dim>::righthandside (std::vector<double> x){
  double value = 0.;
  value = 0.;
  return value;
}
...
template <int dim>
double FEM<dim>::dirichlet_boundary (std::vector<double> x){
  double value = 0., c0 = 1./3., c0_cap = 8.0;
  if (x[1]==0.){
    value = 300.*(1+c0*x[0]);
  }
  if (x[1]==0.08){
    value = 310.*(1+c0_cap*x[0]*x[0]); 
  }
  return value;
}

template <int dim>
double FEM<dim>::neumann_boundary (std::vector<double> x){
  double value = 0.;
  return value;
}