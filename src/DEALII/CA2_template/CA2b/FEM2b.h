...
class FEM
{
 public:
  ...
  //define righthandside and boundary functions
...
std::vector<double> FEM<dim>::basis_gradient(unsigned int node, double xi_1, double xi_2, double xi_3){
  ...
  if(node == 0){
    ...
    std::cout <<  "no such basis function gradient!" << std::endl; 
  }
  //EDIT
...
void FEM<dim>::define_boundary_conds(){
  ...
  for(unsigned int i = 0; i < totalNodes; i++){
    if(nodeLocation[i][0]==0. || nodeLocation[i][0]==0.04){
      for (unsigned int j = 0; j < dim; j++){
        x[j] = nodeLocation[i][j];
      }
  ...
  for (;elem!=endc; ++elem){
    ...
    for(unsigned int q1=0; q1<quadRule; q1++){
      for(unsigned int q2=0; q2<quadRule; q2++){
          ...
          for(unsigned int A=0; A<dofs_per_elem; A++){
            for(unsigned int i = 0; i < dim; i++){
              x[i] += nodeLocation[local_dof_indices[A]][i]*basis_function(A, quad_points[q1], quad_points[q2], quad_points[q3]);
            }
          }
	        for(unsigned int A=0; A<dofs_per_elem; A++){
...
double FEM<dim>::dirichlet_boundary(std::vector<double> x){
  double c0 = 1./3., c0_cap = 8., value = 0.;
  if(x[0]==0.){
    value = 300.*(1.+c0*(x[1]+x[2]));
  }
  if(x[0]==0.04){
    value = 310.*(1.+c0*(x[1]+x[2]));
  }
  return value;
}
