function Poisson_solver_2D(pde, mesh, FE)
% 二维possion方程求解器
...
A = A1+A2;
B = assemble_matrix_volume_in_2D(matrix_size, pde, mesh, FE, 0, 0, 0, 0);
%-----------------------组装右端项量----------------------------------------%
% 组装右端向量
...
[B,b] = treat_Dirichlet_boundary(B, b, boundarynodes,pde.bdry.u_dirichlet,FE.Pb);
%------------------------求解线性方程组-------------------------------------%
% 求解线性方程组
% solution = A\b;
[V, d] = eigs(A, B, 16, 'sm');
